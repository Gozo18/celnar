...
export async function addItemToCart(data: CartItem) {
  try {
    ...
    if (!cart) {
      ...
      return {
        success: true,
        message: `${product.name} byl přidán do košíku`,
      }
    } else {
      ...
      if (existItem) {
        ...
        if (product.stock < existItem.qty + 1) {
          throw new Error("Není dostatek zboží na skladě")
        }
        ...
        // Check stock
        if (product.stock < 1) throw new Error("Není dostatek zboží na skladě")
      ...
      return {
        ...
        message: `${
          existItem
            ? `Přidána další láhev ${product.name}`
...
export async function removeItemFromCart(productId: string) {
  try {
    ...
    // Get user cart
    const cart = await getMyCart()
    if (!cart) throw new Error("Košík nebyl nalezen")
    ...
    return {
      ...
      message: `${
        exist.qty === 1
          ? `${product.name} byl odstraněn z košíku`
          : `Láhev ${product.name} odebrána z košíku`
      }`,
    }
  } catch (error) {
    return {
